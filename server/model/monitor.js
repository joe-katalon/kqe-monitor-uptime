...
class Monitor extends BeanModel {
    ...
    start(io) {
        ...
        const beat = async () => {
            ...
            // Fetch placeholder values once per beat execution
            const placeholderValues = await getEnvPlaceholderValues();
            ...
            try {
                if (await Monitor.isUnderMaintenance(this.id)) {
                    ...
                    if (this.auth_method === "basic") {
                        // Consider if basic_auth_user/pass also need placeholder replacement
                        // For now, assuming they don't. If they do, apply replacePlaceholdersInTarget here.
                        // let processedUser = replacePlaceholdersInTarget(this.basic_auth_user, placeholderValues);
                        // let processedPass = replacePlaceholdersInTarget(this.basic_auth_pass, placeholderValues);
                        // basicAuthHeader = {
                        //    "Authorization": "Basic " + this.encodeBase64(processedUser, processedPass),
                        // };
                        basicAuthHeader = {
                    ...
                    let bodyValue = null;
                    let processedBodyString = this.body; // Original body string

                    // Replace placeholders in the raw body string first
                    if (typeof processedBodyString === "string") {
                        processedBodyString = replacePlaceholdersInTarget(processedBodyString, placeholderValues);
                    }


                    if (processedBodyString && (typeof processedBodyString === "string" && processedBodyString.trim().length > 0)) {
                        if (!this.httpBodyEncoding || this.httpBodyEncoding === "json") {
                            try {
                                bodyValue = JSON.parse(processedBodyString);
                                // If bodyValue is an object/array, replace placeholders in its string values recursively
                                bodyValue = replacePlaceholdersInTarget(bodyValue, placeholderValues);
                                contentType = "application/json";
                            } catch (e) {
                                throw new Error("Your JSON body is invalid after placeholder replacement. " + e.message + ". Original: " + this.body + ", Processed: " + processedBodyString );
                            }
                        } else if (this.httpBodyEncoding === "xml" || this.httpBodyEncoding === "text" || this.httpBodyEncoding === "form") {
                            bodyValue = processedBodyString; // Already a string with placeholders replaced
                            if (this.httpBodyEncoding === "xml") {
                                contentType = "text/xml; charset=utf-8";
                            } else if (this.httpBodyEncoding === "form") {
                                contentType = "application/x-www-form-urlencoded";
                            }
                            // For "text", contentType might be text/plain or not set automatically.
                            // If a specific Content-Type is needed for "text", it should be set via custom headers.
                        }
                    }
                    
                    // Process Custom Headers for Placeholders
                    let customHeaders = {};
                    if (this.headers) {
                        try {
                            let parsedHeaders = JSON.parse(this.headers); // this.headers is a JSON string
                            customHeaders = replacePlaceholdersInTarget(parsedHeaders, placeholderValues);
                        } catch (e) {
                            log.warn("monitor", `[${this.name}] Could not parse custom headers JSON: ${this.headers}. Error: ${e.message}`);
                            // Potentially throw an error or proceed without these headers if critical
                        }
                    }

                    // Axios Options
                    const options = {
                        url: replacePlaceholdersInTarget(this.url, placeholderValues), // Also replace in URL
                        ...
                        headers: {
                            ...
                            ...(basicAuthHeader),
                            ...customHeaders // Spread the processed custom headers
                        },
                        maxRedirects: this.maxredirects,
...
// Helper function to get placeholder values
async function getEnvPlaceholderValues() {
    const placeholderPrefix = "UPTIME_KUMA_PLACEHOLDER_";
    const values = {};

    // 1. Load from Environment variables first
    for (const envVar in process.env) {
        if (envVar.startsWith(placeholderPrefix)) {
            const placeholderKey = envVar.substring(placeholderPrefix.length);
            values[placeholderKey] = process.env[envVar];
        }
    }

    // 2. Load from UI-defined global settings and merge/override
    try {
        const uiPlaceholders = await Settings.get("requestPlaceholders");
        if (Array.isArray(uiPlaceholders)) {
            for (const placeholder of uiPlaceholders) {
                if (placeholder.key && typeof placeholder.value === 'string') { // Ensure value is a string
                    values[placeholder.key] = placeholder.value; // UI overrides ENV
                }
            }
        }
    } catch (e) {
        log.error("monitor", "Error loading UI placeholders from settings: " + e.message);
    }
    
    return values;
}

// Recursive function to replace placeholders in a string or an object/array
function replacePlaceholdersInTarget(target, placeholderValues) {
    if (typeof target === 'string') {
        let replacedString = target;
        for (const key in placeholderValues) {
            const regex = new RegExp(`{{\s*${key}\s*}}`, 'g');
            replacedString = replacedString.replace(regex, placeholderValues[key]);
        }
        return replacedString;
    } else if (Array.isArray(target)) {
        return target.map(item => replacePlaceholdersInTarget(item, placeholderValues));
    } else if (typeof target === 'object' && target !== null) {
        const replacedObject = {};
        for (const objKey in target) {
            replacedObject[objKey] = replacePlaceholdersInTarget(target[objKey], placeholderValues);
        }
