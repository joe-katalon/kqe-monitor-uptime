...
class Monitor extends BeanModel {
                ...
                if (await Monitor.isUnderMaintenance(this.id)) {
                    ...
                    if (this.type === "http") {
                        ...
                        if (data.includes(this.keyword)) {
                            ...
                            throw new Error(
                                bean.msg + ", but keyword is not in [" + data + "]"
                            );
                        }
                    }
    ...
    static async sendStats(io, monitorID, userID) {
        ...
        if (hasClients) {
            ...
            await Monitor.sendUptime(24 * 7, io, monitorID, userID);
    ...
    static async sendNotification(isFirstBeat, monitor, bean) {
        if (!isFirstBeat || bean.status === DOWN) {
            ...
            const uptime24h = await this.calcUptime(24, monitor.id);
            const cleanUptime24h = parseFloat(uptime24h.toPrecision(4));
            const uptime30d = await this.calcUptime(24 * 30, monitor.id);
            const cleanUptime30d = parseFloat(uptime30d.toPrecision(4));
            const displayPercent24h = `${(cleanUptime24h * 100).toFixed(2)}%`;
            const displayPercent30d = `${(cleanUptime30d * 100).toFixed(2)}%`;
            ...
            let msg = `- [${monitor.name}] [${text}] ${bean.msg}\r- ℹ Uptime last 24h: ${displayPercent24h}\r- ℹ Uptime last 30d: ${displayPercent30d}`;
            // let msg = ` [${monitor.name}] [${text}] ${bean.msg} Uptime last 24h: ${cleanUptime24h} Uptime last 30d: ${cleanUptime30d}`;
